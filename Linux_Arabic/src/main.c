...
static void xStudio_clear_line_enter(char *str)
{
...
static int  xStudio_fread_line(FILE *fp, char *dest)
{
	if ((fgets(dest, 10240, fp)) == NULL)
		return -1;
	//xStudio_clear_string_enter(dest);
	xStudio_clear_line_enter(dest);
	return 0;
}
...
int main()
{
	...
	if (xStudio_fread_line(fd, utf8_str) == -1){
		fprintf(stderr, "%s():read file line failed\n", __FUNCTION__);
		return 0;
	}
	//unicode_str = xStudio_string_utf8_to_unicode(utf8_str, &len);
	unicode_str = xStudio_string_utf8_to_unicode(src_srt, &len);
